...
Enemy.prototype.movements = {
	...
	'track':new Movement(function(self){//self = the 'this' parameter
		...
		//if we reached the point, remove it from our wayPoints...
		//some margin of error is allowed or the condition will rarely/never be met
		if(Util.Number.inRange(self.x,self.wayPoints[0].x-self.width*2,self.wayPoints[0].x+self.width*2)&&
			Util.Number.inRange(self.y,self.wayPoints[0].y-self.height*2,self.wayPoints[0].y+self.height*2)
		){
			 self.wayPointReached = true;
		}
		
		function setWaypoint(collision){	
			if(collision&&self.wayPointReached){	
					//if way point is something other than the ship
					//return false
					//if(self.wayPoints[0].type!="target") return false;
					var newPoint = AI.getRandomPointBehindObject(self);
					newPoint.type = "random";
					self.wayPoints[0] = newPoint;
					console.log('random waypoint set');
			}else{
				if(!self.wayPoints[0]){
					self.wayPoints[0] = {x:ship.x,y:ship.y,type:'target'};
				}
				//if the enemy is trying to navigate away from the ship, don't reassign it to the ship
				if(self.wayPoints[0].type=="random"&&!self.wayPointReached) return false;
				self.wayPoints[0] = {x:ship.x,y:ship.y,type:'target'};
				console.log('waypoint to ship set');
			}
